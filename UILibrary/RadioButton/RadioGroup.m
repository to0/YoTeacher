    ...
    for (UIView *control in self.subviews) {
        if ([control isKindOfClass:[RadioBox class]]) {
            ...
            else
            {
                ...
                //[self willChangeValueForKey:@"selectValue"];
                self.selectValue = ((RadioBox*)control).value;
                //printf("select value %ld\n", (long)self.selectValue);
                //[self didChangeValueForKey:@"selectValue"];
            }
        }
    }
    //printf("select value %ld\n", (long)self.selectValue);
...
{
    if ([key isEqualToString:@"selectValue"]) {
        return YES;
    }
