...
static CGFloat const kDefaultContentScrollViewMaxHeight = 150.;
...
- (CGFloat)heightForContentScrollView
{
    //NSLog(@"scroll height: %f\n", MAX(self.contentScrollViewMinHeight, MIN(self.contentScrollViewMaxHeight, CGRectGetHeight(self.contentView.bounds))));
    return MAX(self.contentScrollViewMinHeight, MIN(self.contentScrollViewMaxHeight, CGRectGetHeight(self.contentView.bounds)));
}
...
- (void)updateContentScrollView
{
    ...
    if (_contentView) {
        ...
        if (CGRectGetWidth(_contentView.frame) < self.containerWidth) {
            ...
            //printf("container %f %f\n", self.containerWidth, _contentView.bounds.size.width);
        }
        printf("container %f %f\n", self.containerWidth, _contentView.bounds.size.height);
